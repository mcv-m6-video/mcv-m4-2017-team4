function [sequence_db]=compute_task_1(color_space,sequence_db,groundtruth_path,groundtruth_name,input_path,input_name,task,P_param)
      ...
      %test part
       for count_alpha =1:size(sequence_part.alpha,2)
          ...
          for count_test=1:size(frames_sequence,2)
              ...
              if task==1
                test_image_res=imfill(uint8(test_image_res),8);  
              elseif task==2
                test_image_res=bwareaopen(test_image_res,P_param);  
              elseif task==3
                  test_image_res=imopen(test_image_res,strel('disk',5));
                  test_image_res=imclose(test_image_res,strel('disk',15));
                  test_image_res=bwareaopen(test_image_res,P_param); 
